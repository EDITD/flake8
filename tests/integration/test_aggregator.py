...
def mock_config_finder(program='flake8', arguments=[],
    ...
    config_finder.local_config_files = mock.Mock(return_value=local_configs)

    return config_finder


@pytest.mark.parametrize('config_finder_params,assertions', [
    # Default values with no config file
    ({}, [
        (lambda opts: set(opts.select), {'E', 'W', 'F', 'C90'}),
        (lambda opts: set(opts.ignore), set(defaults.IGNORE)),
        (lambda opts: opts.max_line_length, 79),
    ]),
    # Correct values with user config only
    ({'user_config': USER_CONFIG}, [
        (lambda opts: set(opts.select), {'E', 'W', 'F', 'C90'}),
        (lambda opts: opts.ignore, ['D203']),
        (lambda opts: opts.max_line_length, 79),
    ]),
    ...
    ({'local_configs': [LOCAL_CONFIG]}, [
        (lambda opts: set(opts.select), {'E', 'W', 'F'}),
        (lambda opts: set(opts.ignore), set(defaults.IGNORE)),
        (lambda opts: opts.max_line_length, 79),
    ]),
...
def test_aggregate_options_resulting_values(
        optmanager, config_finder_params, assertions
):
    ...
    for opts_function, expected_value in assertions:
        assert opts_function(parsed_options) == expected_value
